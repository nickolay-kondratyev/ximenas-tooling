...
status_updated_iso: 2026-09-18T04:45:13Z
type: feature
priority: 2
...
Built as specified. 46 pytest tests pass on pandas 3.0 (clean `pip install .`) and on pandas 2.3 (Colab-like), both with `-W error`.

### Where things live
- `pyproject.toml`: setuptools, src layout, `pandas>=2`, `openpyxl>=3`, extra `dev = [pytest]`.
- `src/ximenas_tooling/excel_combine/`:
  - `column_name_normalizer.py`: `ColumnNameNormalizer.normalize`, the single matching rule.
  - `input_file_finder.py`: `InputFileFinder` (top-level `*.xlsx`, sorted; skips `~$` lock files and the output file).
  - `excel_tab_reader.py`: `ExcelTabReader` → `TabReadResult(table | problems)`. Only the matched tab is parsed.
  - `tab_combiner.py`: `TabCombiner.combine(tables, upstream_problems)` → `CombineResult(frame, tolerated_column_mismatch)`. Pure pandas.
  - `column_mismatch.py`: `ColumnMismatch`, `ColumnMismatchReport`. `combine_validation_error.py`: `CombineValidationError`.
  - `excel_combine_job.py`: `ExcelCombineJob.run`, exported as `ximenas_tooling.excel_combine.run`.
- `notebooks/combine_excel_tabs.ipynb`: thin launcher (nbformat-validated, no outputs).
- `tests/excel_combine/`: fixtures are generated by `WorkbookFixture` (openpyxl) in `tmp_path`.
- `self_work.test.fast.sh` (editable venv `.tmp/venv`) / `self_work.test.full.sh` (clean non-editable install).

### Decisions and assumptions (not in the spec)
- **Header read by hand** (`header=None`, then row 1 is used as the header): pandas' `header=0` silently renames duplicate headers to `Clicks.1`, which would hide the duplicate-column error.
- **Entirely empty columns are dropped**, the same way empty rows are (formatted-but-empty Excel columns). A column with data but an **empty header** is a validation problem that names the Excel column letter.
- **Mismatch details use original header text**: `missing` shows the reference file's text and `unexpected` shows that file's own text. The reference is the first file *successfully read*.
- **Error banner**: `COLUMN MISMATCH — nothing was written.` when there is any mismatch, otherwise `PROBLEMS FOUND — nothing was written.`. Other problems are listed before the mismatch section. The override hint is the last line only when a mismatch exists.
- **Other errors use the same `CombineValidationError`**: missing folder, no input files, and a blank `tab_name`.
- **The output sheet is `tab_name` exactly as typed**, truncated to 31 characters (`EXCEL_SHEET_NAME_MAX_LENGTH`). Truncation only matters when extra spacing in the typed name makes it longer than the real tab name.
- **Console output**: `rows=[n]` for each file, the total, the output path, and a `!!!` WARNING block when running in union mode.
- **Empty input tabs**: a tab with a header but no rows adds 0 rows. A fully empty tab is a problem.
